...
function loadUserCookie() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            cookieElement.textContent = cookie || '未设置';
        } else {
            cookieElement.textContent = '未设置';
        }
    })
    .catch(error => {
        document.getElementById('current-cookie').textContent = '加载失败';
    });
}
...
function updateUserCookie() {
    ...
    if (!newCookie) {
        ...
        messageElement.textContent = '请输入有效的Cookie';
    ...
    .then(data => {
        const messageElement = document.getElementById('cookie-message');
        if (data.status === 'success') {
            messageElement.textContent = 'Cookie更新成功';
            messageElement.className = 'message success';
            document.getElementById('new-cookie').value = '';
            loadUserCookie();
        } else {
            messageElement.textContent = data.message || 'Cookie更新失败';
            messageElement.className = 'message error';
        }
    })
    .catch(error => {
...
function startQuery() {
    ...
    queryStatusElement.textContent = '正在启动查询...';
    ...
    .then(data => {
        if (data.status === 'success') {
            queryStatusElement.textContent = '查询已启动，系统将根据配置的频率自动查询并通知';
            queryStatusElement.className = 'message success';
        } else {
            queryStatusElement.textContent = data.message || '启动查询失败';
            queryStatusElement.className = 'message error';
        }
    })
    .catch(error => {
        queryStatusElement.textContent = '启动查询失败，请检查网络连接';
        queryStatusElement.className = 'message error';
    });
}
...
function searchByName() {
    ...
    if (!keyword) {
        messageElement.textContent = '请输入物品名称后再搜索';
        messageElement.className = 'message error';
        return;
    }

    messageElement.textContent = '正在搜索...';
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            if (names.length === 0) {
                ...
                opt.value = '';
                opt.textContent = '未找到匹配项';
                select.appendChild(opt);
                messageElement.textContent = '未找到匹配项';
                messageElement.className = 'message info';
                return;
            }
            names.forEach(n => {
            ...
            messageElement.textContent = `找到 ${names.length} 个候选，请选择其中一个`; 
            messageElement.className = 'message success';
        } else {
            messageElement.textContent = data.message || '搜索失败';
            messageElement.className = 'message error';
        }
    })
    .catch(err => {
        messageElement.textContent = '搜索请求失败: ' + err.message;
        messageElement.className = 'message error';
    });
}
...
function addToWatchlistByName() {
    ...
    if (!selectedName) {
        messageElement.textContent = '请选择一个候选名称后再添加';
        messageElement.className = 'message error';
        return;
    }
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            const opt = document.createElement('option');
            opt.value = '';
            opt.textContent = '请先搜索并选择';
            select.appendChild(opt);
            // 刷新监控列表
            loadWatchlist();
        } else {
            messageElement.textContent = data.message || '添加失败';
            messageElement.className = 'message error';
        }
    })
    .catch(err => {
        messageElement.textContent = '请求失败: ' + err.message;
        messageElement.className = 'message error';
    });
}
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            if (Object.keys(data.watchlist).length === 0) {
                messageElement.textContent = '监控列表为空';
                messageElement.className = 'message info';
                return;
            }
            ...
            messageElement.textContent = data.message || '获取监控列表失败';
            messageElement.className = 'message error';
        }
    })
    .catch(error => {
...
function deleteWatchlistItem(itemId) {
    ...
    .then(data => {
        const messageElement = document.getElementById('watchlist-message');
        if (data.status === 'success') {
            messageElement.textContent = '监控项已删除';
            messageElement.className = 'message success';
            // 刷新监控列表
            loadWatchlist();
        } else {
            messageElement.textContent = data.message || '删除监控项失败';
            messageElement.className = 'message error';
        }
    })
    .catch(error => {
